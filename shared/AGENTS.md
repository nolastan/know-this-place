...
| `unknowns` | the `<small>` line closing the `.vtl` | a disagreement in the record, left unadjudicated |
| `sources[].cites` | the passage named in the Sources footer line | which paragraph of a source the page rests on |
| *(none — the homepage's `.news-cards`)* | `.news-now`, under the hero | the `historical_record` news entry whose card is on the homepage now, repeated at the top while it is |
| *(none — `shared/nearby.json`)* | `.nearby` | lateral links, generated by `build_link_index.py` |
| *(none — `events/events.json`)* | `.panel-events` in the aside, address and place pages | the events announced at this place in the next 30 days — a venue with a page carries its `path` in `events/venues.json`; see [events/AGENTS.md](../events/AGENTS.md) |
...
mode. A third, `--event` violet, was added for the homepage map's event dots
(issue #414) — it is a map hue only so far, and joins the same rule: no
fourth color without a human's say-so. So:
...
- **`<ktp-copy>`** — wraps a string the reader has to carry elsewhere (a
  referral code) and makes the whole box copy it. The string is the element's
  own `<code>` text, so nothing is revealed: with no JS the code is still
  printed and still selectable in one click. Everything that promises a click —
  the `role="button"`, the `ic-copy` icon, and the pointer and hover that hang
  off the role in CSS — is *added by the script*, never written into the page:
  an affordance that needs the script must not exist without it.
