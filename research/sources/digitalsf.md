...
> - **Coverage:** harvested in full — 59,601 unique records, 2,273 numbered-and-dated candidates. None extracted yet.
...
| 593 | **San Francisco Office of Assessor-Recorder Photographs (SFP 23)** |
...
**Start with SFP 23.** The Office of Assessor-Recorder photographed properties
*for assessment*, so the collection is a per-building record by construction —
the address is the reason the photograph exists, not incidental to it. The two
largest collections are too big for one session; split them by decade.
...
- **Rights are mixed and machine-readable — filter on `540$f`.** Across the full
  harvest, **60% are `In Copyright`** and **40% are `No Copyright – United
  States`**, with 33 undetermined. The
...
**32.9% of records name a person** in `600` or `700` — 19,598 of them. Photographers in
...
- **Coverage:** **the repository is harvested in full** — all five real sets,
  ...
  `basc` 10, before deduplication. **No findings extracted yet** — every one of
  the 2,273 candidates is unmined, and none of the 44 collections has been read.
  Re-run the harvester to pick up records added since; it resumes from the
  stored token rather than re-downloading.

- **Verified:** 2026-08-15 (harvested the repository in full: 59,601 unique
  records, read 2,273 numbered-and-dated candidates out of them, resolved 97 of
  a random 140 to an APN against `sf-eas-addresses`. Established that the
  advertised ~97,000-item figure double-counts overlapping sets, that sequential
  sampling misstates density by threefold, and that the 19th-century tail falls
  under the 1909 renumbering rules. No findings file yet.)
