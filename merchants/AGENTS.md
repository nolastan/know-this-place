...
| `blackbird` | Blackbird, restaurant rewards app | **Not fetched — there is nothing to fetch.** `www.blackbird.xyz`'s sitemap names only about fifty curated `/spots/<slug>` marketing pages nationwide (roughly a dozen in San Francisco), each carrying a structured address in the page's Next.js data rather than a directory listing; it is not a way to list the app's San Francisco merchants. Issue #356 typed in 176 listings read out of the app itself, 42 of them with a street address ("addresses past Little Sweet in Financial District will need to be researched" — the issue's own words); the other 134 had their addresses looked up one merchant at a time, off each business's own site or listing, since Blackbird's own site couldn't supply them. The app's `neighborhood` field is frequently wrong or approximate even where the address is well-attested, so it was never treated as evidence | 2,000 $FLY (Blackbird's own points currency) on first check-in, read off the referral link's own landing page — right under the amount the page states "(2,000 Fly is equivalent to $20 USD)", which agrees with the issue's "$20 after first purchase", so the row states it in dollars; the app is mobile-only, so the link opens a download screen rather than the merchant | `blackbird/<date>.json` |
...
| `bounce` | Bounce, luggage storage (its GraphQL API) | `POST https://graphql.usebounce.com/` with `stores(citySlug: "san-francisco")`, paged 50 at a time on `first`/`after` — the order shifts between calls, so the pages overlap and the read de-duplicates on `id`: 115 nodes, 108 distinct. The list answers `address` as empty strings; the per-spot query, `store(slug:)`, carries `gmbAddress`, and only 29 of the 108 have one — the rest have no address anywhere public, and their `/stores/<city>/<slug>` pages print no "Getting there" block either. **The host business is never named**: `name` is the constant "J&J's" on all 108, `description` is empty, and the store page says "this Retail Store" and "the Bounce Partner". So the entries are Bounce's own — it keeps a Google Business listing, phone and opening hours at each address — named "Bounce", and the shop hosting the counter is not named at all. The name is the bare brand because the panel prints `kinds` directly under it, so "Bounce Luggage Storage" over "Luggage storage" said it twice. `nickname` is a wayfinding label and `type` a category for the host; neither reaches a page, and the 26 spots typed "The UPS Store" would need UPS's own locator to be filed under that name. `robots.txt` allows `/stores/*` and disallows `/s/*` | $5 off a first booking, read off `https://bounce.com/ls/coupons` — the referral link itself sits under the disallowed `/s/*` and was never fetched. The friend's booking must be at least $10 to earn it. The link opens the referral screen rather than the spot, so the row carries a `note` | `bounce/<date>.json` |
...
| `instacart` | Instacart, grocery delivery app | **Not fetched.** `https://www.instacart.com/robots.txt` ends in `User-Agent: *` / `Disallow: /`, a blanket disallow, so the listings are the ones typed into issue #281 off `https://www.instacart.com/grocery-delivery/ca/near-me-in-san-francisco-ca` — a name and an address each, and nothing more. No categories and no hours, so no entry publishes `kinds` or `opening_hours` and no panel carries a “Last updated” line; a refresh is a human handing over a fresh list, never a re-fetch | $10 credit, read off the referral link's own landing modal (“You got $10 referral credit!”), which agrees with the issue. The code only takes on the home page, so the row carries a `note` | `instacart/<date>.json` |
...
- **The offer's terms come from the programme's own page, not the ticket.**
  ...
  first booking, and it is the page the reader lands on, so the page wins. A
  ticket's amount is a claim about someone else's programme, and only the
  credential itself is evidence.
...
- **A directory that forbids automated access can still be workable.** `robots.txt` is honoured here as it is in research and news, and Instacart's ends in a blanket disallow — so nothing is fetched from it, then or later. That alone does not make the ticket `needs-human`: the second half a ticket needs is *a way to list the directory's San Francisco locations*, and a person typing them in is one of them. What it costs is everything else the listing page would have carried — categories, hours, and with the hours the panel's “Last updated” line — and it costs the refresh, which becomes a person handing over a fresh list. Read `robots.txt` before the first fetch, not after, and say in the source row that a refresh means asking someone.
...
- **A delivery tier is not a second business at the door.** Instacart lists
  Safeway and “Safeway Rapid” at 2300 16th Street, the second at the same
  address with a synthetic “Suite A20” appended, and does the same for
  Target, FoodsCo and Andronico's. The suffix says how fast the app will
  deliver, not what trades from the building, so the plain listing is the
  entry and the tier is `publish.status: "declined"` with the reason. It is
  the one place the duplicate rule's “keep the fuller name” does not hold:
  the extra words are the directory's, not the shop's.
