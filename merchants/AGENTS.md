...
| `blackbird` | Blackbird, restaurant rewards app | **Not fetched — there is nothing to fetch.** `www.blackbird.xyz`'s sitemap names only about fifty curated `/spots/<slug>` marketing pages nationwide (roughly a dozen in San Francisco), each carrying a structured address in the page's Next.js data rather than a directory listing; it is not a way to list the app's San Francisco merchants. Issue #356 typed in 176 listings read out of the app itself, 42 of them with a street address ("addresses past Little Sweet in Financial District will need to be researched" — the issue's own words); the other 134 are still missing one and need a human, the same gap the issue flags | 2,000 Fly (Blackbird's own points currency) toward meals on first check-in, read off the referral link's own landing page — the issue said "$20 after first purchase", which the page does not say; the app is mobile-only, so the link opens a download screen rather than the merchant | `blackbird/<date>.json` |
