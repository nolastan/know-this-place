...
What it checks:
  ...
  * venues.json — every venue has a name, coordinates come in pairs inside
    ...
    points at a page that actually exists (a place page's place.json, or the
    data.json of the parcel the venue claims to be);
...
def is_page(path: str) -> bool:
    """An address page (data.json) or a place page (place.json)."""
    d = REPO / path.strip("/")
    return (d / "data.json").exists() or (d / "place.json").exists()
...
def check_venues():
    ...
    for key, v in (data.get("venues") or {}).items():
        ...
        if v.get("path") and not is_page(v["path"]):
...
def check_events(sources):
    ...
    for i, ev in enumerate(doc.get("events") or []):
        ...
        if (lat is None) != (lng is None):
            ...
            err(where, f"({lat}, {lng}) is outside San Francisco")
        if ev.get("path") and not is_page(ev["path"]):
