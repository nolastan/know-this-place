...
def resolve(events: list, today: date) -> tuple:
    ...
    beyond the horizon; dedupe what two calendars both list."""
    venues = load_venues()
...
def main() -> int:
    ...
    # A single-source run refreshes that source's slice of the file, not the
    # whole file — a half-fetched events.json is a worse state than a stale one.
    if args.source and OUT.exists():
        ...
        try:
            ...
            events = sorted(events + kept,
                            key=lambda e: (e["start"], e["title"]))
            print(f"  (merged with {len(kept)} events from other sources)")
        except (OSError, json.JSONDecodeError):
            pass
