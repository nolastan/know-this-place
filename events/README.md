...
screen, a listed event is a kept event, and the daily run is three commands of
plain code. The judgement it does need — which place a new venue string
means — is the `/events` skill's.

An event surfaces three ways:

- **on the venue's page**, as an "Upcoming events" panel for the next 30
  days — a place page for a park or plaza, an address page for a building —
  whenever the venue has one;
- **on the homepage map**, as a pulsing violet dot for the next 6 days, which
  opens the venue's page;
