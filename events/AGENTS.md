...
looks: a panel on the building's page while the venue is a parcel the site
documents, a pulsing dot on the homepage map for the next six days, and a
listing on `/events` for everything upcoming.
...
keeps everything they list — the pipeline is mechanical end to end, which is
why GitHub Actions can run it without an agent in the loop. Read the root
...
| 3 build | `scripts/build_events.py`, `scripts/seed_pages.py render` | `events.json` | `events/index.html`, `shared/events.geojson`, address-page panels |
...
There is no judgement stage and no queue: a listed event is a kept event.
That is deliberate, and it is what the issue asked for — the refresh can run
in Actions as plain code, with no model in the loop.
...
- **`venues.json`** is the venue register — how a calendar's free-text
  location becomes a named place with coordinates and, where the place is a
  parcel the site documents, the `path` of its page. Matching is substring on
  the normalized location, longest matcher first. **The `path` rule is the
  important one:** set it only where the venue *is* the documented parcel —
  EAS or the page's own data should say so. A concert on the plaza in front
  of a building is an event at the plaza; it does not belong on the
  neighbouring parcel's page unless the parcel and the venue are the same
  ground. A venue with no page is not a failure — it still gets a dot and a
  listing.
- **`venue-manifest.json`** is how a named venue *gets* a page: the parcel
  list `scripts/seed_pages.py seed-list` ran for the first batch of venues.
  When a new venue is a real parcel the site hasn't documented — a park, a
  plaza, a campus — resolve it (parcels/acdm-wktn `intersects(shape, ...)` on
  the venue pin gives the APN and its official addresses), add it here, and
  re-run `seed-list`; existing pages are always left alone. The exceptions
  are genuine, not gaps: a venue in the public right of way (a stairway, an
  intersection, a block's party) has no parcel and never gets a page, and a
  pin that lands on a *neighbouring* residential parcel is never that
  parcel's page to borrow.
