...
"Upcoming events" panel on the page of the park, plaza or building the event
is at (the next 30 days), a pulsing dot on the homepage map (the next 6, in
the violet `--event` hue) that opens that page, and a listing on `/events`.
The daily read is mechanical — `events.json` is the committed data file
`events/tools/fetch.py` writes and `scripts/build_events.py` builds from — so
its Actions run is code with no model in the loop. What takes judgement —
naming a venue the register doesn't know, pointing it at its page, dropping a
listing that names a private person — is the `/events` skill's, run by hand. An event is not a `historical_record` entry: it leaves no
